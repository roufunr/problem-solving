package leetcode.daily_challenge.may_to_july_24.min_cost_to_convert_string_I;
...
public class Solution {
    ...
    public long minimumCost(String source, String target, char[] original, char[] changed, int[] cost) {
        ...
        for (int i = 0; i < original.length; i++) {
            pathDist[original[i] - 'a'][changed[i] - 'a'] = Math.min(cost[i],
                    pathDist[original[i] - 'a'][changed[i] - 'a']);
        }
        // floyd warshall
        for (int k = 0; k < 26; k++) {
            for (int i = 0; i < 26; i++) {
